...
export async function getPlayerGuild(db: Database, playerId: string): Promise<any | null> {
...
export async function getGuildMembers(db: Database, guildId: string): Promise<any[]> {
...
export async function recomputeGuild(db: Database, guildId: string) {
...
export async function listGuilds(db: Database): Promise<any[]> {
  const { results } = await db.prepare(`
...
export async function addGuildContribution(db: Database, playerId: string, xp: number) {
